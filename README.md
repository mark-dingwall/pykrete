...
pinned to a tag, and still needs config and a key (see Quickstart below).
...
Shell env still takes precedence over `.env` if both are set.
...
| Variable | Purpose |
|----------|---------|
| `NANOGPT_API_KEY` | Required. From the shell environment or a `.env` file in the working directory (shell wins if both are set). |
